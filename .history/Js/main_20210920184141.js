...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        ...
        if($(this).scrollTop() >= 90){
            $("#goToTop").show("slow");
            $(".list").css({"position": "fixed", "top": "50px", "right": "7.5%", "z-index": "9", "width": "17%"})
        }
        else{
            $("#goToTop").hide("slow");
            $(".list").css({"position": "relative", "top": "0px", "right": "0", "width": "20%"})
        }
        $("div.list > ul > li:last-child").hide()
        if($(this).scrollTop() >= 300){
            $("div.list > ul > li:last-child").fadeIn("slow")
        }
        else
            $("div.list > ul > li:last-child").hide("slow")
    })

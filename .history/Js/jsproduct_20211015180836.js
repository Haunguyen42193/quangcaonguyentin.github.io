$(document).ready(function() {
    ...
    $(".foot-product a, .sub-pd a").click(function(){
        ...
        var content = document.querySelectorAll(".foot-product a")
        var content1 = document.querySelectorAll(".sub-pd a")
        for( var j = 0; j < 4; j++)
        {
            alert($(content[j]).attr("href"))
            if($(a[j]).attr("href") === $(content[j]).attr("href") || $(a[j]).attr("href") === $(content1[j]).attr("href"))
            {
                alert("5")
                $(a[j]).addClass("active")
            }
